...
themselves on first run.
    ...
    mhrt-1.40.0.jar        Minecraft 26.2
    mhrt-1.40.0+26.3.jar   Minecraft 26.3

## New in 1.40.0

**Moving things keep their history.** Every accumulator in the denoiser used to
find this frame's pixel in the last frame by projecting it through the previous
camera - exact for a world that sits still, wrong for anything that moves
through it. A spinning end crystal's history was looked up at the pixel it had
already left and thrown away, so it was shown at close to a single traced
sample, soft and a frame stale on top of the pipeline's own frame.

Every box now carries the transform it had a frame ago, so the denoiser follows
a surface through the world rather than only following the camera. A crystal,
a walking mob, a swinging chest lid and a dropped item converge the way the
ground they stand on does. Measured on a crystal filling the frame, it moves
2.1 times as much between frames as it did - which is the damping coming off -
and against a fully converged picture of the same pose it is 19% closer.

It costs about a tenth of a millisecond a frame. In a world with nothing moving
in it the picture is unchanged, to four decimal places on every measure.
`mhrt-nomotion` beside options.txt puts the old behaviour back.
...
* **Resolution** in the F6 pane. The traced picture is rendered below your
...
**F6** in game opens the panes: quality preset, resolution, bounces, denoising,
shadows, the colour grade, lights, and how many entities are traced a frame.
...
| file | what it does |
|---|---|
| `mhrt-settings`, `mhrt-grade`, `mhrt-lights` | your saved settings, written by the F6 panes |
